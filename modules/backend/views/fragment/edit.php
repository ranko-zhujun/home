    ...
    <div class="col-md-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                ...
                <?php $form->action = 'index.php?r=backend/fragment/edit' ?>
                ...
                <?= $form->field($model, 'fragmentkey', ['errorOptions' => ['class' => 'error mt-2 text-danger']])?>
                <input name="FormFragment[body]" type="hidden" id="fragment-body">
                <div class="form-group">
                    <label>
                        片段模板
                    </label>
                    <select name="fragmentTemplate" id="fragmentTemplate" class="form-control selectpicker"
                            style="width:100%"
...
<script>
    ...
    function loadTemplate(id) {
        if (id != '0') {
            var text = $("select[id=fragmentTemplate]").find("option:selected").text();
            $("#formfragment-fragmentname").val(text);
            $.post('index.php?r=backend/fragment/gettemplate', {
                'id': id,
    ...
    function loadFragmentBody(id) {
        $.post('index.php?r=backend/fragment/getfragment', {
            'id': id,
            '_csrf': '<?= Yii::$app->request->csrfToken ?>'
        }, function (data) {
            editor.setValue(data);
